...
# RabbitMQ connection parameters - get from environment or use defaults
...
queue_name = os.environ.get("QUEUE_NAME", "inbox")
routing_key = os.environ.get("ROUTING_KEY", "inbox_message")
api_endpoint = os.environ.get(
    "API_ENDPOINT", "http://localhost:5006/api/inbox/create"
)
...
def callback(ch, method, properties, body):
    """Process incoming messages and forward to API endpoint"""
    try:
        print(f"Received message: {body}")
        message = json.loads(body)

        # Forward the message to the API endpoint
        response = requests.post(
            api_endpoint, json=message, headers={"Content-Type": "application/json"}
        )

        if response.status_code == 201:
            print(f"Successfully forwarded message to API: {response.json()}")
        ...
        # Negative acknowledgment if processing fails
        ch.basic_nack(delivery_tag=method.delivery_tag, requeue=True)
